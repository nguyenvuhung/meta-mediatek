...
copy_cfg_files () {
    for type in ${UBOOT_CONFIG}; do
        cp ${MTK_UBOOT_CFGDIR}/mt7623-bpi-r2/mt7623-bpi-r2-${type}.h ${S}/include/configs
        cp ${MTK_UBOOT_CFGDIR}/mt7623-bpi-r2/mt7623-bpi-r2-${type}-env.txt ${S}
    done
}
    ...
    if [ -n "${UBOOT_CONFIG}" ]; then
        ...
        for config in ${UBOOT_MACHINE}; do
            ...
            for type in ${UBOOT_CONFIG}; do
                j=$(expr $j + 1);
                if [ $j -eq $i ]; then
                    ${B}/${config}/tools/mkenvimage -s 16384 -o u-boot-env-mt7623-bpi-r2-${type}.bin ${S}/mt7623-bpi-r2-${type}-env.txt
                fi
            done
            unset  j
        done
        ...
        tools/mkenvimage -s 16384 -o u-boot-env-mt7623-bpi-r2.bin ${S}/mt7623-bpi-r2-env.txt
    fi
}
...
COMPATIBLE_MACHINE = "(mt7623-evb|mt7623-bpi-r2|mt7623-bpi-r2_dev)"
