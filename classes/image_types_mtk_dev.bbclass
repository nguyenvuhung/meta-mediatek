...
generate_scatter_file () {
    ...
    sed -i "s/__UBOOT_ENV_FILE__/u-boot-env-mt7623-bpi-r2-emmc.bin/g" ${SCATTER_TPL}
...
assemble_sd_image () {
    ...
    dd if=${DEPLOY_DIR_IMAGE}/u-boot-env-mt7623-bpi-r2-sd.bin of=${SD_FILE} bs=1 seek=`printf "%d" ${EMMC_UBOOT_ENV_OFFSET}` conv=notrunc
